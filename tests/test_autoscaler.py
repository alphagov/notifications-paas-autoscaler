...
class TestAutoscaler:
    ...
    def test_run_task(self, mock_statsd_client, mock_cf):
        mock_cf.return_value.get_paas_apps.return_value = {
            'app1': {'name': 'app1', 'instances': 3, 'guid': 'notify-test-app1'},
        }
        mock_autoscaler_app = Mock()
        mock_autoscaler_app.name = 'app1'
        mock_autoscaler_app.get_desired_instance_count.return_value = 5

        autoscaler = Autoscaler()
        autoscaler._schedule = Mock()
        autoscaler.autoscaler_apps = [mock_autoscaler_app]

        autoscaler.run_task()
        mock_statsd_client.gauge.assert_called_once_with("{}.instance-count".format('app1'), 5)
        mock_cf.return_value.apps._update.assert_called_once_with('notify-test-app1', {'instances': 5})
...
'''
        ...
        mock_autoscaler_statsd_client.gauge.assert_called_once_with("{}.instance-count".format(app_name), 6)
        ...
        mock_autoscaler_statsd_client.reset_mock()
        mock_cf.return_value.apps._update.reset_mock()

        autoscaler.run_task()

        mock_autoscaler_statsd_client.gauge.assert_called_once_with("{}.instance-count".format(app_name), 8)
